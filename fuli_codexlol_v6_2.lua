-- FULI HUB - v6.2 Final (ESP dinámico, WalkSpeed 50, stamina fix)
...
addToggle("WalkSpeed 100", speed100,
    function()
...
-- WalkSpeed 50
local speed50 = {active = false}
addToggle("WalkSpeed 50", speed50,
    function()
        speed50.loop = RunService.RenderStepped:Connect(function()
            local char = LocalPlayer.Character
            if char then
                local hum = char:FindFirstChildWhichIsA("Humanoid")
    ...
    function()
        if speed50.loop then speed50.loop:Disconnect() end
        local char = LocalPlayer.Character
        if char then
            local hum = char:FindFirstChildWhichIsA("Humanoid")
            if hum then hum.WalkSpeed = 16 end
